...
<body>
    ...
    preg_match('/\A[0-9]{3}\-[0-9]{4}/u', $address, $matched, PREG_OFFSET_CAPTURE);
    // 123-4567がマッチする
    print_r($matched);

    echo '●文字列から16進数表記のカラーコードを見つける', PHP_EOL;
    $string = 'dark-violet - 濃いすみれ色 - #9400d3a';
    ...
    if (preg_match('/\A[0-9\-]+\z/u', $input)) {
        ...
        echo '半角英数字のみで入力してください。', PHP_EOL; // こちらが出力される
    }

    echo '●半角英数字およびハイフンのみかを入力チェックする', PHP_EOL;
    $input = 'HeLLo123';
    ...
    echo '●半角英数字およびハイフンのみかを入力チェックする（i修飾子）', PHP_EOL;
    $input = '03-９９９９-0000';
    if (preg_match('/\A[a-z0-9]+\z/i', $input)) {
        echo '入力に問題ありません。', PHP_EOL; // こちらが出力される
    } else {
        echo '半角英数字のみで入力してください。', PHP_EOL;
    }
