...
/**
 * 契約期限から１年以上経過した顧客をデータベースから削除するバッチ処理。
 ...
 *  の引数指定時、データベース削除処理を実行せず、削除対象の顧客IDを出力するのみに留めます。
 * 
 * ●-c または --customer-id
 *  一部の顧客IDのみを対象に処理するための引数。この引数がない場合は全顧客を対象に処理します。
 *  以下が使用例です。
 * 
 *  --customer-id=1234
 *  --customer-id=1234,2345,3456
 *  --customer-id=1001-1500
 */

class ExpiredCustomerDeleteBatch
{
    /**
     * コマンドライン引数を格納した連想配列
     */
     ...
     */
    private function parseCustomerIds(): void
    {
        $customerIds = '';
...
$batch = new ExpiredCustomerDeleteBatch($options);
$batch->exec();
